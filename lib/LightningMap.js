  ...
  _createClass(Map, [{
    ...
    value: function updateZoom() {
      if (this.options.zoom !== this.state.targetZoom) {
        ...
        var newZoom = percentage >= 0.99 ? this.state.targetZoom : this.state.startZoom + newZoomDiff;
    ...
    value: function calculateGrid() {
      var centerY = _TileConversion.default.lat2tile(this.options.center[0], Math.round(this.options.zoom), false);

      var centerX = _TileConversion.default.lon2tile(this.options.center[1], Math.round(this.options.zoom), false);
      ...
      for (var y = 0; y < verticalTiles; y++) {
        for (var x = 0; x < horizontalTiles; x++) {
          ...
          if (tileX >= 0 && tileY >= 0) {
            var tile = new _Tile.default(tileX, tileY, Math.round(this.options.zoom));
    ...
    value: function drawTiles() {
      var canvasWidth = this.state.canvasDimensions[0];
      var canvasHeight = this.state.canvasDimensions[1];
