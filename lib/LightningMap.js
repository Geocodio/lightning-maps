...
function () {
  ...
  _createClass(Lightning, [{
    ...
    value: function getTilesCount(canvasSize) {
      var tilesCount = Math.ceil(canvasSize / this.options.tileSize) * this.options.tileAreaMultiplier;
    ...
    value: function updateMoveOffset() {
      ...
      if (targetMoveOffsetChanged) {
        ...
        var targetHasBeenReached = Object.values(this.state.moveOffset).join(',') === Object.values(targetMoveOffset).join(',');
    ...
    value: function updateZoom() {
      if (this.options.zoom !== this.state.targetZoom) {
        ...
        var percentage = this.easeOutQuad(progress / this.options.animationDurationMs);
        var zoomDiff = Math.abs(this.state.targetZoom - this.state.startZoom);
        var newZoomDiff = zoomDiff * percentage;
        var isZoomInOperation = this.state.targetZoom > this.state.startZoom;
        var newZoom = isZoomInOperation ? this.state.startZoom + newZoomDiff : this.state.startZoom - newZoomDiff;
        ...
        var nominator = isZoomInOperation ? 1 : -1;
        var scale = nominator - (this.state.targetZoom - this.options.zoom);
    ...
    value: function calculateGrid() {
      var centerY = _TileConversion.default.lat2tile(this.options.center[0], Math.floor(this.options.zoom), false);

      var centerX = _TileConversion.default.lon2tile(this.options.center[1], Math.floor(this.options.zoom), false);
      ...
      var verticalTiles = this.getTilesCount(this.state.canvasDimensions[1]); // noinspection JSSuspiciousNameCombination
      ...
      for (var y = 0; y < verticalTiles; y++) {
        for (var x = 0; x < horizontalTiles; x++) {
          ...
          if (tileX >= 0 && tileY >= 0) {
            var tile = new _Tile.default(tileX, tileY, Math.floor(this.options.zoom));
    ...
    value: function ensureTileAsset(tile) {
      ...
      var expandTilesOnLoad = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
      ...
      if (!(tile.id in this.state.tiles)) {
        ...
        this.state.tiles[tile.id].onload = function () {
          _this2.state.tiles[tile.id].loaded = true;
        };
      }
    ...
    value: function garbageCollect() {
      ...
      if (allTiles.length > this.maxTilesToKeep()) {
        ...
        tilesToConsider.splice(tilesToConsider.length - tilesToDeleteCount).forEach(function (tile) {
          tile.src = '';
          delete _this3.state.tiles[tile.tileId];
        });
      }
    }
  }, {
    ...
    value: function drawTiles() {
      var tileSize = this.options.tileSize * this.state.scale;
      ...
      var horizontalOverflow = horizontalTiles * tileSize - this.state.canvasDimensions[0];
      var verticalOverflow = verticalTiles * tileSize - this.state.canvasDimensions[1];
      ...
      for (var y = 0; y < verticalTiles; y++) {
        for (var x = 0; x < horizontalTiles; x++) {
          ...
          if (tile) {
            var tileX = this.state.moveOffset[0] + centerOffset[0] + (x * tileSize - horizontalOverflow / 2);
            var tileY = this.state.moveOffset[1] + centerOffset[1] + (y * tileSize - verticalOverflow / 2);
    ...
    value: function drawMarkers() {
      ...
      visibleMarkers.map(function (marker) {
        var position = _TileConversion.default.latLonToPixel(marker.coords, _this4.options.center, _this4.options.zoom, _this4.options.tileSize, _this4.state.canvasDimensions);

        marker.render(_this4.context, [center[0] - position[0] + _this4.state.moveOffset[0], center[1] - position[1] + _this4.state.moveOffset[1]]);
      });
    }
  }, {
    key: "addMarker",
