  ...
  _createClass(Map, [{
    ...
    value: function attachEvents() {
      ...
      this.canvas.addEventListener('dblclick', function (event) {
        ...
        _this.setTargetMoveOffset(-(event.clientX - canvasCenter[0]), -(event.clientY - canvasCenter[1]));
    ...
    value: function handleMouseEventInteraction(event, name) {
      ...
      var anyItemIsHover = controlObjects.length > 0 || markers.length > 0;
      var polygons = [];

      if (!anyItemIsHover) {
        var mapState = new _MapState__WEBPACK_IMPORTED_MODULE_2__["default"](this.options.center, this.options.zoom, this.state.targetZoom, this.options.tileSize, this.state.canvasDimensions, this.state.moveOffset);
        polygons = this.state.polygons.map(function (polygon) {
          return polygon.handleMouseOver(_this5.context, mapState, _this5.state.mousePosition);
      ...
      this.canvas.style.cursor = anyItemIsHover ? 'pointer' : 'grab';
...
function () {
  ...
  _createClass(Polygon, [{
    ...
    key: "projectedGeometryState",
    get: function get() {
      return this._projectedGeometryState;
    },
    set: function set(projectedGeometryState) {
